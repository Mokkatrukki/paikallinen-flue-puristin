...
3. For each picked track, call apple_music_search_tracks (artist + track name). Before using a result, check that its returned "name" and "artistName" fields actually correspond to the track you searched for — a result for a different song or a different artist entirely is NOT a match, even if it's the top hit (this happens: a search can silently return an unrelated song with no error). If the top result doesn't match, try once more with a simplified query (e.g. drop a parenthetical qualifier like "(CL)" from the artist name, or drop minor words from the track title) before giving up. If no result matches after that retry, do NOT skip the track silently and do NOT substitute a different song — leave it out of the playlist and call it out by name in your final report as "not found on Apple Music", so it can be added manually.
...
5. Call apple_music_create_playlist with name "New Music #<n+1>" (the next number up) and all of this run's successfully found trackIds included directly. Every run makes a brand new playlist with the next number — never add tracks to an existing "New Music #..." playlist, and never reuse or skip a number.
6. Report which albums contributed which tracks, which (if any) you skipped because nothing was clearly praised, which (if any) picked tracks could not be found on Apple Music, and the name of the playlist you created.`;
}
